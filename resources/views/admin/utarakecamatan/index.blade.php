...
@section('sub-judul','Tambah Dokumen Akta Kematian Payakumbuh Utara')
@section('content')
...
@endif
  ...
  <div class="row">
	<form action= "/utarakecamatan" method="GET">
	<a href="{{ route('Utarakecamatan.create') }}" class="btn btn-info btn-sm">Tambah Akta Kematian</a>
...
<br><br>
	...
	<table class="table table-striped table-hover table-sm table-bordered">
		...
		<tbody>
			@foreach ($utarakecamatan as $result => $hasil)
			<tr>
				<td>{{ $result + $utarakecamatan->firstitem() }} </td>
				<td>{{ optional($hasil->utarakel)->utarakel}}</td>
				...
				<td>
			<form action="{{ route('Utarakecamatan.destroy', $hasil->id ) }}" method="POST"> 
						@csrf
						@method('delete')
						<a href="{{ route('Utarakecamatan.edit', $hasil->id ) }}" class="btn btn-primary btn-sm">Edit</a>
