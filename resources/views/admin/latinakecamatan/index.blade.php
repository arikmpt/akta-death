...
@section('sub-judul','Tambah Dokumen Akta Kematian Payakumbuh Latina')
@section('content')
...
@endif
  ...
  <div class="row">
	...
	<a href="{{ route('latinakecamatan.create') }}" class="btn btn-info btn-sm">Tambah Akta Kematian</a>
...
<br><br>
	...
	<table class="table table-striped table-hover table-sm table-bordered">
		...
		<tbody>
			@foreach ($latinakecamatan as $result => $hasil)
			<tr>
				<td>{{ $result + $latinakecamatan->firstitem() }} </td>
				<td>{{ optional($hasil->latinakel)->latinakel}}</td>
				...
				<td>
			<form action="{{ route('latinakecamatan.destroy', $hasil->id ) }}" method="POST"> 
						@csrf
						@method('delete')
						<a href="{{ route('latinakecamatan.edit', $hasil->id ) }}" class="btn btn-primary btn-sm">Edit</a>
